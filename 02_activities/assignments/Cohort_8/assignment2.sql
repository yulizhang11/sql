...
FROM customer_purchases
GROUP BY product_id, customer_id; -- not sure if this satisfy the prompt because I didn't use window function 
...
SELECT DISTINCT
product_name
,TRIM(SUBSTR(product_name,NULLIF(INSTR(product_name,'-'),0)+2,10)) as description
FROM product;
...
-- not sure the correct REGEXP to capture a numerical value 
...
SELECT market_date
,total_sales
,row_number() OVER(ORDER BY total_sales ASC) as row_num
...
SELECT * FROM best_day WHERE row_num = 1 
UNION 
SELECT * FROM worst_day WHERE row_num = 1;
...
vendor_name
,product_name
,original_price * 5  * 26 as profit
FROM new_vendor_inventory nvi
INNER JOIN vendor v
	ON nvi.vendor_id = v.vendor_id
INNER JOIN product p
	ON nvi.product_id = p.product_id; 
-- not really sure why we need to use cross join in this case, I know that for each product i am selling 5 times to 26 customers 
-- so each vendor makes original_price mulitpy by 5 and then 26 
...
SET snapshot_timestamp = datetime('now','localtime');
...
VALUES(24, 'Green Onion', 'bunch', 1, 'unit', datetime('now','localtime'));
...
SELECT DISTINCT vendor_id, product_id FROM vendor_inventory
ORDER BY vendor_id;
